...
TMP="$(mktemp -d /tmp/opencode/m43dom.XXXXXX)"
DB="$TMP/oj.db"
PORT="${OJ_FRONTEND_PORT:-$(( (RANDOM % 2000) + 21000 ))}"
export OJ_JWT_SECRET="m43-dom-verification-secret-0123456789"
...
BASE="http://127.0.0.1:$PORT" JSDOM_DIR="$JSDOM_DIR" \
  node "$ROOT/tests/frontend/m43_problem_page_dom.mjs"
RC=$?
if [ "$RC" -ne 0 ]; then
