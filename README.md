...
后续阶段（Rejudge、完整沙箱/资源限制/watchdog 硬上限、CodeMirror、完整搜索筛选分页页面）尚未实现。
...
真实 C++17/C11 并发判题结果正确且子进程无遗留、优雅停止排空已接收任务。
...
- **停止策略**：`shutdown()` 先停止接收新任务并唤醒等待线程，然后**排空**（drain）所有
  已接收任务、投递其结果后再回收 worker；`HttpServer::stop()` 先停止 HTTP（含正在同步
  等待的请求）再停止调度器，保证关闭数据库前没有 worker 仍在使用数据库，且不遗留
  永不完成的结果通道。
- **配置与限制**：`OJ_JUDGE_QUEUE_CAPACITY` 见「配置方式」。M3.1 未实现 M3.2 的
  watchdog 全局硬上限与强制终止、M3.3 沙箱、M3.4 完整分类/Sanitizer/内存采集、
  M3.6 Rejudge；停止时排空已接收任务可能等待较久（受现有单次判题保护超时约束），
  强制终止与完整资源清理留待后续阶段。
...
前台运行时按 `Ctrl+C`（或发送 `SIGTERM`）即可优雅停止：服务会先停止监听，再停止
判题调度器（不再接收新任务，执行完所有已接收任务后回收 worker），最后关闭数据库，
保证无永久等待、无未回收 worker、无数据库关闭顺序错误。验证方式：
